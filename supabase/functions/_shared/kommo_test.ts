...
Deno.test('Kommo provider base URL accepts only account subdomain under kommo.com', () => {
  assertKommoBaseUrl('https://pierriquintproducoes.kommo.com/api/v4')
  for (const value of [
    'https://evil.example/api/v4',
    'https://pierriquintproducoes.kommo.com.evil.example/api/v4',
    'https://pierriquintproducoes.kommo.com/api/v4/../v3',
    'http://pierriquintproducoes.kommo.com/api/v4',
  ]) {
    let denied = false
    try { assertKommoBaseUrl(value) } catch (error) {
      denied = error instanceof Error && error.message === 'KOMMO_BASE_URL_DENIED'
    }
    if (!denied) throw new Error(`unsafe Kommo URL accepted: ${value}`)
  }
})
...
Deno.test('global Kommo contact identity is exact phone match, independent of lead stage', () => {
  const contact = { id: 1, custom_fields_values: [
...
Deno.test('email alone does not match when caller requests phone-only identity', () => {
  const contact = { id: 1, custom_fields_values: [{ field_code: 'EMAIL', values: [{ value: 'a@b.com' }] }] }
...
Deno.test('stage mapping prioritizes reschedule event then appointment status', () => {
  const settings = {
    pipeline_id: 10, stage_awaiting_payment_id: 11, stage_confirmed_id: 12,
    stage_rescheduled_id: 13, stage_cancelled_id: 14, stage_completed_id: 15,
    stage_no_show_id: 16, stage_expired_id: 17,
  }
...
Deno.test('missing stage fails closed', () => {
  let failed = false
  try {
    stageIdForAppointment({ pipeline_id: 10, stage_awaiting_payment_id: null, stage_confirmed_id: null,
      stage_rescheduled_id: null, stage_cancelled_id: null, stage_completed_id: null,
      stage_no_show_id: null, stage_expired_id: null }, 'CONFIRMED', 'UPDATED')
  } catch (error) { failed = error instanceof Error && error.message.startsWith('KOMMO_STAGE_NOT_CONFIGURED') }
...
Deno.test('reservation Data field is resolved uniquely and must be date-compatible', () => {
  const id = findUniqueLeadDateFieldId([{ id: 10, name: 'Pai', type: 'text' }, { id: 20, name: 'Data', type: 'date' }])
  if (id !== 20) throw new Error('Data field id mismatch')
  let failed = false
  try { findUniqueLeadDateFieldId([{ id: 30, name: 'Data', type: 'text' }]) }
  catch (error) { failed = error instanceof Error && error.message === 'KOMMO_RESERVATION_DATE_FIELD_INVALID_TYPE' }
...
Deno.test('lead card field mapping fails closed on ambiguity or incompatible type', () => {
  let ambiguous = false
  try {
    resolveLeadCardFields([{ id: 1, name: 'Data', type: 'date' }, { id: 2, name: 'Saldo', type: 'numeric' },
      { id: 3, name: 'Saldo', type: 'numeric' }, { id: 4, name: 'Extras locação', type: 'text' }])
  } catch (error) { ambiguous = error instanceof Error && error.message === 'KOMMO_BALANCE_FIELD_AMBIGUOUS' }
  ...
  } catch (error) { invalidExtras = error instanceof Error && error.message === 'KOMMO_RENTAL_EXTRAS_FIELD_INVALID_TYPE' }
