...
function firstOrderPayment(raw: Record<string, unknown>): { payment: Record<string, unknown> | null; count: number } {
  ...
  return { payment: payments.length > 0 ? object(payments[0]) : null, count: payments.length }
}
...
export function buildMercadoPagoWebhookManifest(dataId: string, requestId: string | null | undefined, timestamp: string): string {
...
export async function verifyMercadoPagoWebhookSignature(input: { signature: string; requestId?: string | null; dataId: string; secret: string }): Promise<boolean> {
  ...
  if (normalizedId !== input.dataId) {
    return verifyHmac(input.secret, parsed.v1, buildMercadoPagoWebhookManifest(input.dataId, input.requestId, parsed.ts))
  }
  return false
}
...
function orderStatusForClient(status: string | null | undefined, statusDetail?: string | null): MercadoPagoPaymentSnapshot['status'] {
  ...
  if (raw === 'expired' || ((raw === 'canceled' || raw === 'cancelled') && detail === 'expired')) return 'expired'
...
export function sanitizeMercadoPagoPayment(raw: Record<string, unknown>): MercadoPagoPaymentSnapshot {
  ...
  return {
    ...
    point_of_interaction: qrCode || qrBase64 || ticketUrl ? {
      transaction_data: { qr_code: qrCode, qr_code_base64: qrBase64, ticket_url: ticketUrl },
    } : undefined,
    three_ds_info: challengeUrl ? { external_resource_url: challengeUrl } : undefined,
  }
}
...
export function assertMercadoPagoPaymentMatchesIntent(snapshot: MercadoPagoPaymentSnapshot, expected: MercadoPagoExpectedIntent): void {
  if (!snapshot.id) throw new Error('MERCADO_PAGO_PAYMENT_ID_MISSING')
  if (snapshot.provider_transaction_count !== 1 || !snapshot.provider_transaction_id) throw new Error('MERCADO_PAGO_PAYMENT_METHOD_MISMATCH')
  if (!expected.transactionId || snapshot.external_reference !== expected.transactionId) throw new Error('MERCADO_PAGO_EXTERNAL_REFERENCE_MISMATCH')
  if (moneyToCents(snapshot.transaction_amount) !== moneyToCents(expected.cashAmount)) throw new Error('MERCADO_PAGO_PAYMENT_AMOUNT_MISMATCH')
  if (expected.method === 'PIX') {
    if ((snapshot.payment_method_id ?? '').toLowerCase() !== 'pix' || (snapshot.payment_type_id ?? '').toLowerCase() !== 'bank_transfer') {
      throw new Error('MERCADO_PAGO_PAYMENT_METHOD_MISMATCH')
    }
    return
  }
  if ((snapshot.payment_type_id ?? '').toLowerCase() !== 'credit_card') throw new Error('MERCADO_PAGO_PAYMENT_METHOD_MISMATCH')
  if (expected.providerPaymentMethodId && snapshot.payment_method_id && snapshot.payment_method_id.toLowerCase() !== expected.providerPaymentMethodId.toLowerCase()) {
    throw new Error('MERCADO_PAGO_PAYMENT_METHOD_MISMATCH')
  }
}

export function validateCardSubmission(value: unknown, maxInstallments: number): { token: string; paymentMethodId: string; installments: number; issuerId: string | null } {
