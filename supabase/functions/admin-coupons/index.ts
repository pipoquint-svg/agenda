...
function numeric(value: unknown): number {
  const next = Number(value)
  if (!Number.isFinite(next)) throw new Error('COUPON_DISCOUNT_VALUE_INVALID')
  return next
}
function timestamp(value: unknown): string | null {
  if (value === null || value === undefined || value === '') return null
  const next = text(value)
  if (!next || Number.isNaN(Date.parse(next))) throw new Error('COUPON_VALIDITY_INVALID')
  return new Date(next).toISOString()
}
function uuidArray(value: unknown): string[] {
  if (!Array.isArray(value)) throw new Error('COUPON_SERVICES_INVALID')
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'DELETE') {
      const { data, error } = await client.rpc('admin_remove_coupon_audited', {
        p_coupon_id: uuid(body.coupon_id), p_admin_id: admin.adminId,
      })
    ...
    const args = {
      p_code: text(body.code),
      p_discount_type: text(body.discount_type).toUpperCase(),
      p_discount_value: numeric(body.discount_value),
      p_valid_from: timestamp(body.valid_from),
      p_valid_until: timestamp(body.valid_until),
      p_max_uses: integer(body.max_uses, 'max_uses', true),
      p_max_uses_per_customer: integer(body.max_uses_per_customer, 'max_uses_per_customer', true),
      p_customer_id: uuid(body.customer_id, true),
      p_service_ids: uuidArray(body.service_ids ?? []),
      p_admin_id: admin.adminId,
    }

    if (req.method === 'POST') {
      const { data, error } = await client.rpc('admin_create_coupon_audited', args)
      if (error) throw new Error(error.message)
      return json(data, 201)
    }

    const { data, error } = await client.rpc('admin_update_coupon_audited', {
      ...args,
      p_coupon_id: uuid(body.coupon_id),
