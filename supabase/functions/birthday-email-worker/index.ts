...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!envEnabled('BIRTHDAY_EMAIL_DELIVERY_ENABLED')) {
      return json({ ok: true, skipped: true, reason: 'BIRTHDAY_EMAIL_DELIVERY_DISABLED' })
    }
    if (!envEnabled('TRANSACTIONAL_EMAIL_ENABLED')) {
      return json({ ok: true, skipped: true, reason: 'TRANSACTIONAL_EMAIL_DISABLED' })
    }
    if (!envEnabled('NOTIFICATION_TEMPLATES_RUNTIME_ENABLED')) {
      return json({ ok: true, skipped: true, reason: 'NOTIFICATION_TEMPLATES_RUNTIME_DISABLED' })
    }
    ...
    const { data: logs, error: logsError } = await client
      ...
      .select('id,template_id,customer_id,status,attempt_count,idempotency_key,payload_snapshot')
      .eq('event_key', 'BIRTHDAY')
      .eq('channel', 'EMAIL')
    ...
    for (const log of logs ?? []) {
      const payloadSnapshot = (log.payload_snapshot && typeof log.payload_snapshot === 'object')
        ? log.payload_snapshot as Record<string, unknown>
        : {}
      const scope = String(payloadSnapshot.operation_scope ?? '').trim().toUpperCase()
      const cycleId = String(payloadSnapshot.birthday_cycle_id ?? '').trim()
      const couponId = String(payloadSnapshot.coupon_id ?? '').trim()
      ...
      if (!['BLACKSHEEP', 'SABRINA'].includes(scope) || !cycleId || !log.template_id || !log.customer_id) {
        await client.from('notification_delivery_logs').update({
          ...
          last_error_code: 'BIRTHDAY_DELIVERY_CONTEXT_INVALID',
      ...
      const [{ data: template, error: templateError }, { data: customer, error: customerError }, { data: coupon, error: couponError }, { data: operationSettings, error: operationError }] = await Promise.all([
        client.from('notification_template_configs')
          ...
          .maybeSingle(),
        client.from('customers').select('id,name,email').eq('id', log.customer_id).maybeSingle(),
        couponId
      ...
      if (templateError || customerError || couponError || operationError || !template || !customer) {
        await client.from('notification_delivery_logs').update({
          attempt_count: Number(log.attempt_count ?? 0) + 1,
          last_error_code: 'BIRTHDAY_DELIVERY_CONTEXT_LOOKUP_FAILED',
          updated_at: new Date().toISOString(),
        }).eq('id', log.id)
        failed += 1
        continue
      ...
      if (!template.is_active || template.event_key !== 'BIRTHDAY' || template.channel !== 'EMAIL' || template.audience !== 'CUSTOMER' || (template.operation_scope && template.operation_scope !== scope)) {
        await client.from('notification_delivery_logs').update({
          status: 'FAILED',
          attempt_count: Number(log.attempt_count ?? 0) + 1,
          last_error_code: 'BIRTHDAY_TEMPLATE_INVALID',
          updated_at: new Date().toISOString(),
        }).eq('id', log.id)
        failed += 1
        continue
      ...
      if (couponId && (!coupon || coupon.source !== 'BIRTHDAY' || coupon.customer_id !== customer.id || !coupon.is_active)) {
        await client.from('notification_delivery_logs').update({
          status: 'FAILED',
          attempt_count: Number(log.attempt_count ?? 0) + 1,
          last_error_code: 'BIRTHDAY_COUPON_INVALID',
          updated_at: new Date().toISOString(),
        }).eq('id', log.id)
        failed += 1
        continue
      ...
      if (!recipient || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(recipient)) {
        await Promise.all([
          client.from('notification_delivery_logs').update({
            status: 'SKIPPED',
            attempt_count: Number(log.attempt_count ?? 0) + 1,
            last_error_code: 'EMAIL_RECIPIENT_MISSING_OR_INVALID',
            updated_at: new Date().toISOString(),
          }).eq('id', log.id),
          client.from('birthday_automation_cycles').update({ message_status: 'SKIPPED', updated_at: new Date().toISOString() }).eq('id', cycleId),
        ])
        continue
      }
      ...
      const values: Record<string, string> = {
        ...
        'coupon.discount': '',
        'coupon.expires_at': formatDate(coupon?.valid_until),
      ...
      try {
        ...
        await client.from('notification_delivery_logs').update({
          status: 'FAILED',
          attempt_count: Number(log.attempt_count ?? 0) + 1,
          last_error_code: error instanceof Error ? error.message.split(':')[0].slice(0, 120) : 'BIRTHDAY_TEMPLATE_RENDER_FAILED',
          updated_at: new Date().toISOString(),
        }).eq('id', log.id)
        failed += 1
        continue
      ...
      try {
        ...
        const [deliveryUpdate, cycleUpdate] = await Promise.all([
          client.from('notification_delivery_logs').update({
            status: 'SENT',
            attempt_count: Number(log.attempt_count ?? 0) + 1,
          ...
          client.from('birthday_automation_cycles').update({ message_status: 'SENT', updated_at: now }).eq('id', cycleId),
        ])
        if (deliveryUpdate.error || cycleUpdate.error) throw new Error('BIRTHDAY_DELIVERY_STATUS_UPDATE_FAILED')

        await client.from('audit_logs').insert({
          entity_type: 'BIRTHDAY_AUTOMATION',
          entity_id: cycleId,
        ...
        await Promise.all([
          client.from('notification_delivery_logs').update({
            ...
            last_error_code: error instanceof Error ? error.message.split(':')[0].slice(0, 120) : 'EMAIL_PROVIDER_FAILED',
            updated_at: new Date().toISOString(),
          }).eq('id', log.id),
          terminal
            ? client.from('birthday_automation_cycles').update({ message_status: 'FAILED', updated_at: new Date().toISOString() }).eq('id', cycleId)
    ...
    return json({ ok: true, skipped: false, selected: logs?.length ?? 0, sent, deferred, failed })
  } catch (error) {
    ...
    const status = message.startsWith('GITHUB_') ? 401 : 500
