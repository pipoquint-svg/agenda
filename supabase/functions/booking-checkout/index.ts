...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const internalCode = knownCode ?? code.split(':')[0]
    const publicCode = publicCustomerFieldError(internalCode)
    const status = ['RATE_LIMITED', 'CUSTOMER_VERIFICATION_RATE_LIMITED', 'CUSTOMER_VERIFICATION_COOLDOWN'].includes(internalCode) ? 429
      : internalCode === 'RATE_LIMIT_BACKEND_FAILED' ? 503
      : ['CHECKOUT_HOLD_NOT_ACTIVE', 'RESOURCE_NOT_AVAILABLE', 'HOLD_SELECTION_REQUIRES_NEW_SLOT', 'COUPON_USAGE_LIMIT_REACHED', 'COUPON_CUSTOMER_USAGE_LIMIT_REACHED'].includes(internalCode) ? 409
      : internalCode.startsWith('EMAIL_') || internalCode === 'CUSTOMER_VERIFICATION_DELIVERY_FAILED' ? 503
      : 400
    await recordOpsEdgeFailure(adminClient, 'booking-checkout', internalCode, status, !knownCode)
