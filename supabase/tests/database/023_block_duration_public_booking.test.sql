...
-- Tuesday 2030-01-01. The service may be contracted until 14:00, while the
-- studio resource remains available until 14:30 so the single final buffer fits.
...
select is(
  (public.public_get_booking_page('blacksheep')->'services'->0->>'duration_mode'),
  'BLOCKS',
  'public catalog exposes block-duration mode'
);

select is(
  (public.public_get_booking_page('blacksheep')->'services'->0->>'booking_block_minutes')::integer,
  30,
  'public catalog exposes 30-minute rental blocks'
);
...
select is(
  ...
  120,
  'four blocks quote two contracted hours'
);
...
select is(
  ...
  360.00::numeric(12,2),
  'four blocks price four block units'
);

select throws_ok(
  $$ select public.public_quote_booking_duration(
    'blacksheep',
    '97100000-0000-0000-0000-000000000005',
    '97100000-0000-0000-0000-000000000006',
    1, '[]'::jsonb, 1
  ...
  'public request cannot bypass minimum rental blocks'
);

select is(
  (select count(*)::integer from public.public_list_available_slots_duration(
    'blacksheep',
    '97100000-0000-0000-0000-000000000005',
    '97100000-0000-0000-0000-000000000006',
    8, '[]'::jsonb, 1, '2030-01-01'::date
  )),
  5,
  '4h rental offers 08:00, 08:30, 09:00, 09:30 and 10:00 starts'
);

select ok(
  exists (
    select 1 from public.public_list_available_slots_duration(
      'blacksheep',
      '97100000-0000-0000-0000-000000000005',
      '97100000-0000-0000-0000-000000000006',
      8, '[]'::jsonb, 1, '2030-01-01'::date
    ...
    where s.core_start_at = '2030-01-01 08:00:00-03'::timestamptz
      ...
      and s.slot_end_at = '2030-01-01 12:00:00-03'::timestamptz
  ),
...
select is(
  (select (payload->>'duration_blocks')::integer from block_hold),
  4,
  'hold snapshots four rental blocks'
);

select is(
  (select (payload->>'contracted_minutes')::integer from block_hold),
  120,
  'hold snapshots 120 contracted minutes'
);
...
select is(
  (select upper(ra.occupied_range)
   ...
   where h.id = (select (payload->>'checkout_hold_id')::uuid from block_hold)
     and ra.resource_id = '97100000-0000-0000-0000-000000000002'),
  '2030-01-01 10:30:00-03'::timestamptz,
...
select ok(
  not exists (
    select 1 from public.public_list_available_slots_duration(
      ...
      '97100000-0000-0000-0000-000000000005',
      '97100000-0000-0000-0000-000000000006',
      2, '[]'::jsonb, 1, '2030-01-01'::date
    ) s
    where s.core_start_at = '2030-01-01 10:00:00-03'::timestamptz
  ),
...
select ok(
  exists (
    ...
    where s.core_start_at = '2030-01-01 10:30:00-03'::timestamptz
  ),
...
select is(
  ...
  210,
  'six blocks create 3h contract plus exactly one 30m buffer'
);
...
select is(
  ...
  270,
  'eight blocks create 4h contract plus exactly one 30m buffer'
);
