...
select is((select count(*)::integer from public.appointment_overdue_balances where appointment_id='96500000-0000-0000-0000-000000000008'),1,'expired serviced reservation enters overdue filter');
select is((public.service_admin_reissue_balance_collection('96500000-0000-0000-0000-000000000008','96500000-0000-0000-0000-000000000002')->>'sequence')::integer,2,'first administrative reissue is allowed');
select is((select status from public.appointment_balance_collections where appointment_id='96500000-0000-0000-0000-000000000008' and sequence=2),'PENDING','reissue creates fresh pending collection');

-- NO_SHOW is financially treated as a performed rental and remains collectible.
insert into public.appointments(
 id,public_code,service_id,service_employee_id,service_name_snapshot,primary_customer_id,status,financial_status,
 start_at,end_at,core_start_at,core_end_at,duration_minutes,contracted_minutes,pre_service_minutes,post_service_minutes,people_count,
 commercial_value,billing_mode_snapshot,confirmation_percentage_snapshot
...
select is(public.enqueue_due_rental_balance_collections(),1,'NO_SHOW creates balance collection because rental is considered performed');
...
-- INVOICE has its own billing lifecycle.
insert into public.appointments(
 id,public_code,service_id,service_employee_id,service_name_snapshot,primary_customer_id,status,financial_status,
 start_at,end_at,core_start_at,core_end_at,duration_minutes,contracted_minutes,pre_service_minutes,post_service_minutes,people_count,
 commercial_value,billing_mode_snapshot,confirmation_percentage_snapshot
...
-- Partial in-person payment creates a new collection for the reduced balance.
...
insert into public.appointments(
 ...
 '96500000-0000-0000-0000-000000000012','BAL-PART','96500000-0000-0000-0000-000000000005','96500000-0000-0000-0000-000000000006','Locação BlackSheep','96500000-0000-0000-0000-000000000007','CONFIRMED','PARTIALLY_PAID',
...
select is(public.enqueue_due_rental_balance_collections(),1,'partial-payment scenario starts with live collection');
select is((public.service_record_manual_contract_payment('96500000-0000-0000-0000-000000000012','96500000-0000-0000-0000-000000000002',100,'CASH','127.0.0.1','pgTAP','request-partial')->>'balance_after')::numeric,400::numeric,'partial in-person payment reduces balance immediately');
select is((select payment_discount_amount from public.payment_transactions where appointment_id='96500000-0000-0000-0000-000000000012' and provider='MANUAL' order by created_at desc limit 1),0::numeric,'manual payment never receives PIX discount');
select is((public.service_mark_balance_collection_cancelled((select id from public.appointment_balance_collections where appointment_id='96500000-0000-0000-0000-000000000012' and sequence=1),'PARTIAL','96500000-0000-0000-0000-000000000002','127.0.0.1','pgTAP','request-partial-cancel')->>'status'),'CANCELLED_PARTIAL_PAYMENT','old collection receives explicit terminal state after partial payment');
select is((public.service_admin_reissue_balance_collection('96500000-0000-0000-0000-000000000012','96500000-0000-0000-0000-000000000002')->>'amount')::numeric,400::numeric,'new collection snapshots reduced balance');

select set_config('agenda.test_now','2026-09-01 10:01:00-03',true);
select is(public.expire_due_balance_collections(),3,'all due pending collections expire deterministically');
...
select is(public.expire_due_balance_collections(),1,'second reissue can expire');
select throws_ok($$select public.service_admin_reissue_balance_collection('96500000-0000-0000-0000-000000000012','96500000-0000-0000-0000-000000000002')$$,'P0001','BALANCE_COLLECTION_REISSUE_LIMIT_REACHED','third administrative reissue is refused');
