...
select ok(
  ...
  'service role can read Amelia history after admin authentication'
);
...
select is(
  jsonb_typeof(public.service_admin_list_agenda('2040-01-01 00:00:00-03'::timestamptz, '2040-01-02 00:00:00-03'::timestamptz)->'external_blocks'),
  'array',
  'operational agenda always returns an external blocks array'
);
...
select ok(
  pg_get_functiondef('public.service_admin_list_agenda(timestamptz,timestamptz)'::regprocedure) like '%duration_blocks%',
  'admin agenda exposes block-duration snapshot'
);

select ok(
  pg_get_functiondef('public.service_admin_list_agenda(timestamptz,timestamptz)'::regprocedure) like '%contracted_minutes%',
  'admin agenda exposes contracted minutes separately from buffers'
);
