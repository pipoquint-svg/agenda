...
select has_function('public','service_admin_assert_financial_settlement_permission',array['uuid'],'financial decision permission guard exists');
select has_trigger('public','appointment_policy_actions','appointment_policy_actions_financial_settlement_guard','cancellation settlement trigger exists');
select throws_ok(
  ...
  'operation user cannot explicitly divert a customer return to customer balance'
);
select lives_ok(
  $$select public.service_admin_assert_financial_settlement_permission('21000000-0000-4000-8000-000000000001'::uuid)$$,
  'finance-authorized owner can record an explicit customer-balance choice'
);
