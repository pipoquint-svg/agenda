...
values ('97700000-0000-0000-0000-000000000004','97700000-0000-0000-0000-000000000003','Refund Service','refund-service',120,1000,1,10);
insert into public.service_employees(id,service_id,employee_id)
values ('97700000-0000-0000-0000-000000000005','97700000-0000-0000-0000-000000000004','97700000-0000-0000-0000-000000000002');
...
values ('97700000-0000-0000-0000-000000000004',48,0,20,30,30);
...
values
('97700000-0000-0000-0000-000000000010','REFUND-MIXED-1','97700000-0000-0000-0000-000000000004','97700000-0000-0000-0000-000000000005','CONFIRMED','PARTIALLY_PAID','2035-03-10 10:00:00-03','2035-03-10 12:00:00-03',120,1,'97700000-0000-0000-0000-000000000001',1000,now()),
('97700000-0000-0000-0000-000000000020','REFUND-MP-ONLY-1','97700000-0000-0000-0000-000000000004','97700000-0000-0000-0000-000000000005','CONFIRMED','PARTIALLY_PAID','2035-03-11 10:00:00-03','2035-03-11 12:00:00-03',120,1,'97700000-0000-0000-0000-000000000001',1000,now());

insert into public.payment_transactions(id,appointment_id,transaction_type,method,provider,provider_payment_id,status,contract_amount_settled,payment_discount_amount,cash_amount,paid_at,payment_purpose)
values
('97700000-0000-0000-0000-000000000011','97700000-0000-0000-0000-000000000010','CHARGE','PIX','MERCADO_PAGO','mp-mixed-charge','APPROVED',500,25,475,'2035-03-01 10:00:00-03','CONTRACT'),
('97700000-0000-0000-0000-000000000012','97700000-0000-0000-0000-000000000010','CHARGE','TRANSFER','MANUAL',null,'APPROVED',100,0,100,'2035-03-01 11:00:00-03','CONTRACT'),
('97700000-0000-0000-0000-000000000021','97700000-0000-0000-0000-000000000020','CHARGE','PIX','MERCADO_PAGO','mp-only-charge','APPROVED',500,25,475,'2035-03-01 10:00:00-03','CONTRACT');

select has_function('public','service_get_cancellation_refund_plan',array['uuid'],'provider refund plan function exists');
select has_function('public','service_record_cancellation_provider_refund',array['uuid','uuid','text','numeric','jsonb'],'provider refund recorder exists');
...
select lives_ok($$select public.service_record_cancellation_provider_refund(
 (select id from public.appointment_policy_actions where appointment_id='97700000-0000-0000-0000-000000000010' and action_type='CANCEL'),
 '97700000-0000-0000-0000-000000000011','mp-refund-mixed-475',475,'{"id":"mp-refund-mixed-475","amount":475}'::jsonb
...
select lives_ok($$select public.service_record_cancellation_provider_refund(
 (select id from public.appointment_policy_actions where appointment_id='97700000-0000-0000-0000-000000000020' and action_type='CANCEL'),
 '97700000-0000-0000-0000-000000000021','mp-refund-only-475',475,'{"id":"mp-refund-only-475","amount":475}'::jsonb
)$$,'provider-only refund can be recorded');
select is((select status from public.appointment_policy_actions where appointment_id='97700000-0000-0000-0000-000000000020' and action_type='CANCEL'),'REFUNDED','policy action completes only when full eligible cash was refunded');
select is((select financial_status::text from public.appointments where id='97700000-0000-0000-0000-000000000020'),'REFUNDED','fully refunded appointment has REFUNDED financial status');
select ok((public.service_record_cancellation_provider_refund(
 (select id from public.appointment_policy_actions where appointment_id='97700000-0000-0000-0000-000000000020' and action_type='CANCEL'),
 '97700000-0000-0000-0000-000000000021','mp-refund-only-475',475,'{}'::jsonb
)->>'idempotent_replay')::boolean,'replaying same provider refund is idempotent');
select is((select count(*)::integer from public.payment_transactions where appointment_id='97700000-0000-0000-0000-000000000020' and transaction_type='REFUND'),1,'idempotent replay does not duplicate refund transaction');
