-- Sabrina public booking uses the same protected hold-selection update as BlackSheep.
-- People count is collected after the slot is held. The function still rejects any
-- selection update that would change contracted, pre-service, or post-service time.
...
begin
  ...
  if coalesce(v_service.operation_scope,'') not in ('BLACKSHEEP','SABRINA') then
  ...
  select bp.slug
   ...
   where bp.id=v_hold.booking_page_id
     and bp.is_active;
  ...
  if exists(
    select 1
     ...
     where phr.checkout_hold_id=v_hold.id
       and phr.status='HELD'
  ) then
  ...
  select coalesce(
    ...
    from jsonb_to_recordset(coalesce(p_extra_selections,'[]'::jsonb))
         x(extra_id uuid,quantity integer);
  ...
  v_new_contracted:=public.resolve_service_contracted_minutes(
    v_hold.service_id,
    v_hold.duration_blocks
  );
