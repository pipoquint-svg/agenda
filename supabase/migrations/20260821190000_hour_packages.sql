...
create table public.hour_package_usages (
  ...
  check ((status = 'HELD' and checkout_hold_id is not null) or status <> 'HELD')
);
...
as $$
  select greatest(
    0,
    p.purchased_seconds
      + coalesce((select sum(a.delta_seconds) from public.hour_package_adjustments a where a.package_id = p.id), 0)
      - coalesce((
...
begin
  ...
  loop
    perform public.release_hour_package_usage(v_hold.id, 'Checkout hold expired');
