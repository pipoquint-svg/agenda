-- Item D — fiscal/admin-finance read models exclude explicit test data.
...
begin
  ...
  where cbm.direction='CREDIT' and not cbm.is_test and (a.id is null or not a.is_test)
    and coalesce(cbm.expires_at,cbm.created_at+interval '12 months')>=v_start
    and coalesce(cbm.expires_at,cbm.created_at+interval '12 months')<v_end
    and (v_scope is null or s.operation_scope=v_scope);
...
begin
  ...
  ) order by a.start_at desc),'[]'::jsonb) into v_rows
  from public.appointment_policy_actions pa
  join public.appointments a on a.id=pa.appointment_id
  join public.services s on s.id=a.service_id
  left join public.customers c on c.id=a.primary_customer_id
  cross join lateral (select public.service_get_cancellation_refund_plan(pa.id) plan) p
  where not a.is_test and pa.action_type='CANCEL' and pa.settlement_choice='REFUND' and pa.status='PENDING_REFUND'
    and coalesce((p.plan->>'remaining_refund_cash')::numeric,0)>0.009 and (v_scope is null or s.operation_scope=v_scope);
...
begin
  ...
  with raw as (
    ...
    join public.customers c on c.id=a.primary_customer_id
    left join public.services s on s.id=a.service_id
    where not a.is_test and coalesce(a.commercial_value,0)>0 and a.status::text not in('CANCELLED','EXPIRED')
...
begin
  ...
  -- Refund-request rows have no appointment/movement lineage in the current schema.
  -- Existing test inventory contains zero balance movements/refund requests, so this metric is preserved unchanged.
  select coalesce(sum(amount),0)::numeric(12,2) into v_refunds
