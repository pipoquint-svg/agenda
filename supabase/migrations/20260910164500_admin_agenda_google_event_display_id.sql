-- Gestão only: expose the stable Google event id so the admin UI can group
-- one external event that blocks multiple resources without changing any
-- allocation, availability or booking behavior.
...
begin
  ...
  return jsonb_build_object(
    ...
    'external_blocks', coalesce((
      select jsonb_agg(jsonb_build_object(
        'allocation_id', ra.id,
        'google_calendar_event_id', gce.id,
        'resource_id', r.id,
