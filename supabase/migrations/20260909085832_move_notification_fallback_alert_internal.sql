...
create or replace function agenda_internal.record_notification_template_fallback_alert()
returns trigger
language plpgsql
security definer
set search_path = 'public', 'agenda_internal', 'pg_temp'
as $$
declare
  v_scope text;
...
revoke all on function agenda_internal.record_notification_template_fallback_alert() from public, anon, authenticated;

drop function if exists public.record_notification_template_fallback_alert();
...
for each row
execute function agenda_internal.record_notification_template_fallback_alert();
