...
declare
  ...
  v_request_prefix text; v_replay_amount numeric(12,2):=0;
begin
  ...
  if not found or v_action.action_type<>'CANCEL' or v_action.settlement_choice<>'REFUND' then raise exception using errcode='P0001',message='CANCELLATION_REFUND_NOT_PENDING'; end if;

  select coalesce(sum(cash_amount),0)::numeric(12,2) into v_replay_amount
  from public.payment_transactions
  where policy_action_id=p_policy_action_id and transaction_type='REFUND' and status in('APPROVED','REFUNDED')
    and idempotency_key like v_request_prefix||'%';
  if v_replay_amount>0 then
    select id into v_first_id from public.payment_transactions
    where policy_action_id=p_policy_action_id and transaction_type='REFUND' and status in('APPROVED','REFUNDED')
      and idempotency_key like v_request_prefix||'%'
    order by created_at,id limit 1;
    v_after:=public.service_get_cancellation_refund_plan(p_policy_action_id);
    return v_after||jsonb_build_object('refund_transaction_id',v_first_id,'manual_refund_recorded',v_replay_amount,'method',v_method,'paid_at',v_paid_at,'idempotent_replay',true);
  end if;

  if v_action.status<>'PENDING_REFUND' then raise exception using errcode='P0001',message='CANCELLATION_REFUND_NOT_PENDING'; end if;
  ...
  loop
    ...
    values(v_action.appointment_id,'REFUND',v_method,'MANUAL','APPROVED',v_contract_take,v_take,v_parent.id,v_paid_at,p_admin_id,btrim(p_reference),v_request_prefix||v_parent.id::text,p_policy_action_id,'CONTRACT')
  ...
  v_after:=public.service_get_cancellation_refund_plan(p_policy_action_id);
  return v_after||jsonb_build_object('refund_transaction_id',v_first_id,'manual_refund_recorded',v_amount,'method',v_method,'paid_at',v_paid_at,'idempotent_replay',false);
end;
$function$;
