-- Resource availability is fail-closed: a required physical resource is available
-- only inside an explicit weekly availability rule or an explicit OPEN exception.
-- BLOCK exceptions and allocations continue to override availability.
...
begin
  ...
  loop
    ...
    loop
      ...
      -- Fail closed: without an explicit weekly opening window or OPEN exception,
      -- the physical resource is not available.
      if not (
        exists (
          select 1
...
comment on function public.list_available_slots_for_duration_without_google_sync_gate(uuid,uuid,integer,jsonb,integer,date,text) is
  'Availability is the intersection of service employee availability and every required physical resource. Required resources fail closed outside explicit weekly OPEN windows or OPEN exceptions.';
