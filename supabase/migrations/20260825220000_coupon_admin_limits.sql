...
begin
  ...
  select c.max_uses_per_customer into v_limit
  ...
  if v_customer_id is null then
    raise exception using errcode='P0001', message='COUPON_CUSTOMER_REQUIRED';
  end if;

  select count(*)::integer into v_used
  from public.appointment_discounts ad
  join public.appointments a on a.id = ad.appointment_id
  where ad.coupon_id = new.coupon_id
    and a.primary_customer_id = v_customer_id;

  if v_used >= v_limit then
    raise exception using errcode='P0001', message='COUPON_CUSTOMER_USAGE_LIMIT_REACHED';
  end if;
...
create trigger appointment_discounts_coupon_customer_limit
before insert on public.appointment_discounts
for each row execute function public.enforce_coupon_customer_usage_limit();
...
create or replace function public.admin_create_coupon_audited(
  p_code text,
...
declare
  ...
  v_type text := upper(btrim(coalesce(p_discount_type,'')));
begin
  ...
  if exists(select 1 from unnest(coalesce(p_service_ids,'{}'::uuid[])) id where not exists(select 1 from public.services s where s.id=id)) then raise exception using errcode='P0001', message='COUPON_SERVICE_NOT_FOUND'; end if;

  insert into public.coupons(code,discount_type,discount_value,valid_from,valid_until,is_active,source,customer_id,max_uses,max_uses_per_customer,used_count)
  values(v_code,v_type,p_discount_value,p_valid_from,p_valid_until,true,'PROMOTION',p_customer_id,p_max_uses,p_max_uses_per_customer,0)
  returning id into v_id;

  insert into public.coupon_services(coupon_id,service_id)
  select v_id,id from unnest(coalesce(p_service_ids,'{}'::uuid[])) id;

  insert into public.audit_logs(admin_user_id,entity_type,entity_id,action,before_json,after_json,origin)
  values(p_admin_id,'COUPON',v_id,'COUPON_CREATED',null,(select item from jsonb_array_elements(public.admin_list_coupons()) item where item->>'id'=v_id::text limit 1),'ADMIN');

  return (select item from jsonb_array_elements(public.admin_list_coupons()) item where item->>'id'=v_id::text limit 1);
end;
$$;

create or replace function public.admin_update_coupon_audited(
  p_coupon_id uuid,
  p_code text,
  p_discount_type text,
  p_discount_value numeric,
...
begin
  if not public.service_admin_has_permission(p_admin_id,'FINANCE_MANAGE') then raise exception using errcode='P0001', message='ADMIN_PERMISSION_DENIED'; end if;
  select item into v_before from jsonb_array_elements(public.admin_list_coupons()) item where item->>'id'=p_coupon_id::text limit 1;
  if v_before is null then raise exception using errcode='P0001', message='COUPON_NOT_FOUND'; end if;
  ...
  update public.coupons set code=v_code,discount_type=v_type,discount_value=p_discount_value,valid_from=p_valid_from,valid_until=p_valid_until,
    max_uses=p_max_uses,max_uses_per_customer=p_max_uses_per_customer,customer_id=p_customer_id,is_active=coalesce(p_is_active,true),updated_at=now()
  where id=p_coupon_id;
  delete from public.coupon_services where coupon_id=p_coupon_id;
  insert into public.coupon_services(coupon_id,service_id) select p_coupon_id,id from unnest(coalesce(p_service_ids,'{}'::uuid[])) id;
  ...
  if v_before is distinct from v_after then
    insert into public.audit_logs(admin_user_id,entity_type,entity_id,action,before_json,after_json,origin)
    values(p_admin_id,'COUPON',p_coupon_id,'COUPON_UPDATED',v_before,v_after,'ADMIN');
  end if;
  return v_after;
end;
$$;
...
set search_path = public
as $$
declare v_before jsonb; v_used boolean;
begin
  if not public.service_admin_has_permission(p_admin_id,'FINANCE_MANAGE') then raise exception using errcode='P0001', message='ADMIN_PERMISSION_DENIED'; end if;
  select item into v_before from jsonb_array_elements(public.admin_list_coupons()) item where item->>'id'=p_coupon_id::text limit 1;
  if v_before is null then raise exception using errcode='P0001', message='COUPON_NOT_FOUND'; end if;
  perform 1 from public.coupons where id=p_coupon_id for update;
  select exists(select 1 from public.appointment_discounts where coupon_id=p_coupon_id) into v_used;
  if v_used then
    update public.coupons set is_active=false,updated_at=now() where id=p_coupon_id;
    insert into public.audit_logs(admin_user_id,entity_type,entity_id,action,before_json,after_json,origin)
    values(p_admin_id,'COUPON',p_coupon_id,'COUPON_ARCHIVED',v_before,(select item from jsonb_array_elements(public.admin_list_coupons()) item where item->>'id'=p_coupon_id::text limit 1),'ADMIN');
    return jsonb_build_object('coupon_id',p_coupon_id,'removed',false,'archived',true);
  end if;
  delete from public.coupons where id=p_coupon_id;
  insert into public.audit_logs(admin_user_id,entity_type,entity_id,action,before_json,after_json,origin)
  values(p_admin_id,'COUPON',p_coupon_id,'COUPON_DELETED',v_before,null,'ADMIN');
  return jsonb_build_object('coupon_id',p_coupon_id,'removed',true,'archived',false);
end;
$$;
...
grant execute on function public.admin_list_coupons() to service_role;
grant execute on function public.admin_create_coupon_audited(text,text,numeric,timestamptz,timestamptz,integer,integer,uuid,uuid[],uuid) to service_role;
grant execute on function public.admin_update_coupon_audited(uuid,text,text,numeric,timestamptz,timestamptz,integer,integer,uuid,uuid[],boolean,uuid) to service_role;
grant execute on function public.admin_remove_coupon_audited(uuid,uuid) to service_role;
