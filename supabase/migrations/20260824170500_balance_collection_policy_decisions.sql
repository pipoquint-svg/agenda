-- Commercial decisions approved 2026-08-24 for BlackSheep rental balance collection.
-- NO_SHOW is financially treated as a performed rental.
-- Cancellation/reschedule cutoff for BlackSheep duration rentals is 12 hours.
-- Partial in-person payment cancels the live provider charge and reissues for the new balance.
-- Maximum administrative reissues per appointment: 2.
...
alter table public.appointment_balance_collections
  ...
  check (status in (
    'PENDING','PAID','EXPIRED','CANCELLED_SETTLED','CANCELLED_NO_SHOW','CANCELLED_PARTIAL_PAYMENT'
  ));

-- NO_SHOW no longer cancels a balance collection. Historical CANCELLED_NO_SHOW rows remain valid.
drop trigger if exists trg_enqueue_no_show_balance_cancellation on public.appointments;

-- Apply the 12-hour commercial cutoff to existing BlackSheep duration-rental policies.
-- Do not fabricate a policy row when one does not exist; publication hardening remains authoritative.
...
begin
  ...
  if p_source='AUTO_START' and found then
  ...
  values('APPOINTMENT',p_appointment_id,
    ...
    jsonb_build_object(
      ...
      'expires_at',v_collection.expires_at,'reissue_count',v_reissues + case when p_source='ADMIN_REISSUE' then 1 else 0 end
    ),
  ...
  return jsonb_build_object(
    ...
    'status','PENDING','amount',v_balance,'issued_at',v_collection.issued_at,
...
set search_path=public
as $$
declare v_previous public.appointment_balance_collections%rowtype; v_reissues integer;
begin
  ...
  from public.appointment_balance_collections
  where appointment_id=p_appointment_id and source='ADMIN_REISSUE';
  if v_reissues>=2 then raise exception using errcode='P0001',message='BALANCE_COLLECTION_REISSUE_LIMIT_REACHED'; end if;
  select * into v_previous from public.appointment_balance_collections
  where appointment_id=p_appointment_id order by sequence desc limit 1 for update;
  if not found then raise exception using errcode='P0001',message='BALANCE_COLLECTION_REISSUE_NOT_ALLOWED'; end if;
  if v_previous.status='PENDING' then raise exception using errcode='P0001',message='BALANCE_COLLECTION_STILL_ACTIVE'; end if;
  ...
  if exists(
    select 1 from public.payment_transactions
    where balance_collection_id=v_previous.id and provider='MERCADO_PAGO'
      and transaction_type='CHARGE' and status='PENDING' and provider_payment_id is not null
...
set search_path=public
as $$
declare v_now timestamptz:=public.balance_collection_clock(); v_collection public.appointment_balance_collections%rowtype; v_target text; v_action text;
begin
  if p_reason='SETTLED' then v_target:='CANCELLED_SETTLED'; v_action:='BALANCE_COLLECTION_CANCELLED_SETTLED';
  elsif p_reason='PARTIAL' then v_target:='CANCELLED_PARTIAL_PAYMENT'; v_action:='BALANCE_COLLECTION_CANCELLED_PARTIAL_PAYMENT';
  elsif p_reason='NO_SHOW' then v_target:='CANCELLED_NO_SHOW'; v_action:='BALANCE_COLLECTION_CANCELLED_NO_SHOW';
  ...
  values('APPOINTMENT',v_collection.appointment_id,v_action,
...
select a.id appointment_id,a.public_code,a.primary_customer_id customer_id,c.name customer_name,a.service_id,a.service_name_snapshot service_name,s.operation_scope,
 ...
 coalesce((select count(*) from public.appointment_balance_collections rc where rc.appointment_id=a.id and rc.source='ADMIN_REISSUE'),0)::integer reissue_count,
 greatest(2-coalesce((select count(*) from public.appointment_balance_collections rc where rc.appointment_id=a.id and rc.source='ADMIN_REISSUE'),0),0)::integer reissues_remaining
from public.appointments a
join public.services s on s.id=a.service_id
