...
export type DurationBookingPageData = {
  ...
  services: DurationBookingService[]
}
...
export function contractedMinutes(service: DurationBookingService, blocks: number | null): number {
  if (service.duration_mode === 'BLOCKS') {
    return (service.booking_block_minutes ?? 30) * (blocks ?? service.minimum_booking_blocks ?? 1)
  }
...
export async function quoteDurationBooking(input: {
  pageSlug: string
  service: DurationBookingService
  serviceEmployeeId: string
  durationBlocks: number | null
  extras: ExtraSelection[]
  peopleCount: number
}): Promise<BookingQuote> {
  const { data, error } = await supabase.rpc('public_quote_booking_duration', {
  ...
  trackServiceSelected({
    brand: pageCache.get(input.pageSlug)?.brand_key ?? input.pageSlug,
    serviceId: input.service.id,
    serviceName: input.service.name,
    value: startingPrice(input.service),
  })
  trackFunnelStep('booking_duration_selected', {
    ...
    contracted_minutes: contractedMinutes(input.service, input.durationBlocks),
    currency: 'BRL',
...
export async function listDurationBookingSlots(input: {
  ...
  const { data, error } = await supabase.rpc('public_list_available_slots_duration', {
    p_booking_page_slug: input.pageSlug,
    p_service_id: input.service.id,
    p_service_employee_id: input.serviceEmployeeId,
    p_duration_blocks: input.durationBlocks,
