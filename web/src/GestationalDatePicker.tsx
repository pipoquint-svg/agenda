...
function recommendationCopy(window: GestationalRecommendationWindow | null, fallback: boolean): string {
  if (fallback) return 'Não encontramos horários na janela recomendada. Estes são os próximos disponíveis.'
  if (!window) return ''
  if (window.mode === 'NEARBY_AFTER_32') {
    return 'Vamos encontrar uma data mais próxima para você. Primeiro, separamos horários desta semana e da próxima.'
  }
  if (window.currentAge.totalDays < 29 * 7) {
    return 'Separamos algumas opções para quando você estiver entre 29 e 32 semanas.'
  }
  return 'Você já está em uma ótima fase para o ensaio. Separamos os próximos horários antes de 32 semanas.'
}
    ...
    <>
      <div className="sby-form-grid">
        <label className="sby-field sby-field-wide">
          ...
          <small>Usamos esta data apenas para sugerir horários; ela não limita as outras datas da agenda.</small>
        </label>
      </div>
      ...
      {recommendedSlots.length > 0 ? (
        <div className="sby-time-section">
          <div className="sby-time-head">
            <strong>Horários recomendados para você</strong>
      ...
      <div className="sby-actions end">
        <button type="button" className="sby-secondary" onClick={() => setShowCalendar((current) => !current)}>
          {showCalendar ? 'Ocultar calendário' : 'Ver todas as datas'}
        </button>
      </div>
