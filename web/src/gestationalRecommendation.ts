...
/**
 * Calculates gestational age for a local calendar date from DPP (40w0d).
 * This is presentation/recommendation logic only; it never restricts booking.
 */
...
/**
 * BlackSheep Gestante recommendation rule:
 * - before 29w0d: prioritize dates where the client will be 29w0d–31w6d;
 * - 29w0d–31w6d: prioritize remaining dates before 32w0d;
 * - from 32w0d: prioritize the rest of the current Mon–Sun week + next week.
 *
 * Full calendar remains available outside this window.
 */
export function resolveGestationalRecommendationWindow(
  ...
  if (currentAge.totalDays < WEEK_32_DAYS) {
    // 29w0d is 77 days before DPP. 31w6d is 57 days before DPP.
  ...
  const weekday = todayDate.getUTCDay() // Sunday=0, Monday=1
...
/** A bounded fallback that can be sent to the public slot-range RPC. */
