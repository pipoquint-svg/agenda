...
export type DayTimePricingRule = {
  id?: string
  name: string
...
export type ServiceSettings = {
  ...
  price_per_extra_person: number | string
  buffer_before_minutes: number
  buffer_after_minutes: number
  custom_fields: ServiceCustomField[]
  day_time_pricing_rules: DayTimePricingRule[]
...
async function request(path: string, accessToken: string, init?: RequestInit): Promise<Response> {
  ...
  if (!response.ok) {
    ...
    try { const body = await response.json(); code = body?.error?.code ?? code } catch { /* stable fallback */ }
...
export async function loadServiceCatalog(accessToken: string): Promise<ServiceCatalogBundle> {
  ...
  return { services: body.services ?? [], categories: body.categories ?? [], extras: body.extras ?? [] }
}

export async function listServiceSettings(accessToken: string): Promise<ServiceSettings[]> {
  return (await loadServiceCatalog(accessToken)).services
}

export async function createCategory(payload: { name: string; slug: string; operation_scope: OperationScope }, accessToken: string): Promise<void> {
  await request('admin-service-settings', accessToken, { method: 'POST', body: JSON.stringify({ entity: 'CATEGORY', ...payload }) })
}
export async function saveCategory(payload: ServiceCategory, accessToken: string): Promise<void> {
  await request('admin-service-settings', accessToken, { method: 'PUT', body: JSON.stringify({ entity: 'CATEGORY', category_id: payload.id, name: payload.name, slug: payload.slug, operation_scope: payload.operation_scope, sort_order: payload.sort_order, is_active: payload.is_active }) })
}
export async function removeCategory(categoryId: string, accessToken: string): Promise<{ removed: boolean; archived: boolean }> {
  return (await request('admin-service-settings', accessToken, { method: 'DELETE', body: JSON.stringify({ entity: 'CATEGORY', category_id: categoryId }) })).json()
}

export async function createExtra(payload: { name: string; description: string | null; price: number; duration_delta_minutes: number }, accessToken: string): Promise<void> {
  await request('admin-service-settings', accessToken, { method: 'POST', body: JSON.stringify({ entity: 'EXTRA', ...payload }) })
}
export async function saveExtra(payload: ExtraCatalogItem, accessToken: string): Promise<void> {
  await request('admin-service-settings', accessToken, { method: 'PUT', body: JSON.stringify({ entity: 'EXTRA', extra_id: payload.id, name: payload.name, description: payload.description, price: Number(payload.price), duration_delta_minutes: payload.duration_delta_minutes, is_active: payload.is_active }) })
}
export async function removeExtra(extraId: string, accessToken: string): Promise<{ removed: boolean; archived: boolean }> {
  return (await request('admin-service-settings', accessToken, { method: 'DELETE', body: JSON.stringify({ entity: 'EXTRA', extra_id: extraId }) })).json()
}

export async function createService(payload: CreateServicePayload, accessToken: string): Promise<void> {
  await request('admin-service-settings', accessToken, { method: 'POST', body: JSON.stringify({ entity: 'SERVICE', ...payload }) })
}
export async function saveServiceCatalog(payload: UpdateServiceCatalogPayload, accessToken: string): Promise<void> {
  await request('admin-service-settings', accessToken, { method: 'PUT', body: JSON.stringify({ entity: 'SERVICE', ...payload }) })
}
export async function removeService(serviceId: string, accessToken: string): Promise<{ removed: boolean; archived: boolean }> {
  return (await request('admin-service-settings', accessToken, { method: 'DELETE', body: JSON.stringify({ entity: 'SERVICE', service_id: serviceId }) })).json()
}
export async function saveCustomFields(serviceId: string, fields: ServiceCustomField[], accessToken: string): Promise<void> {
  await request('admin-service-settings', accessToken, { method: 'PUT', body: JSON.stringify({ entity: 'SERVICE', service_id: serviceId, action: 'CUSTOM_FIELDS', fields }) })
}
export async function saveDayTimePricing(serviceId: string, rules: DayTimePricingRule[], accessToken: string): Promise<void> {
  await request('admin-service-settings', accessToken, { method: 'PUT', body: JSON.stringify({ entity: 'SERVICE', service_id: serviceId, action: 'DAY_TIME_PRICING', rules }) })
}
export async function saveServiceExtras(serviceId: string, extras: Array<{ extra_id: string; sort_order: number; is_required: boolean; max_quantity: number; schedule_placement: 'PREPEND' | 'APPEND'; default_schedule_minutes: number | null }>, accessToken: string): Promise<void> {
  await request('admin-service-settings', accessToken, { method: 'PUT', body: JSON.stringify({ entity: 'SERVICE', service_id: serviceId, action: 'SERVICE_EXTRAS', extras }) })
}
...
export async function saveChangePolicy(serviceId: string, policy: ChangePolicy, accessToken: string): Promise<void> {
  await request('admin-service-settings', accessToken, { method: 'PUT', body: JSON.stringify({ entity: 'SERVICE', service_id: serviceId, action: 'CHANGE_POLICY', policy }) })
}
